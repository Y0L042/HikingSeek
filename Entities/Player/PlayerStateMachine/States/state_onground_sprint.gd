class_name State_Player_OnGround_Sprint
extends PlayerStateClass

func enter() -> void:
	GDebug.print(player, ["Entered State_Player_OnGround_Sprint"])
	pass
...
func custom_physics_process(_delta: float) -> void:
	...
	player.move(_delta, player.MOVE_STATS.ground_sprint_speed, player.MOVE_STATS.ground_acceleration)
